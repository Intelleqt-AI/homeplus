...
const Calendar = () => {
  ...
  // Real property maintenance events with current dates
  ...
  const filteredEvents = events.filter(event => {
    ...
    if (filterType === 'financial') return event.type === 'admin' || event.title.toLowerCase().includes('insurance') || event.title.toLowerCase().includes('payment');
    if (filterType === 'household') return event.title.toLowerCase().includes('bin') || event.title.toLowerCase().includes('meter') || event.title.toLowerCase().includes('shopping');
  ...
  const upcomingEvents = filteredEvents.filter(event => 
    event.date > new Date() && event.date.toDateString() !== new Date().toDateString()
  ).slice(0, 5);

  // Cost calculations
  const thisMonthCost = filteredEvents.filter(event => {
    const eventDate = new Date(event.date);
    const now = new Date();
  ...
  const yearlyBudget = 4000;

  const getStatusIcon = (status: string) => {
    switch (status) {
      case "confirmed":
        return <><CheckCircle className="w-3 h-3 text-green-600 mr-1" />Confirmed</>;
      case "action_required":
        return <><AlertTriangle className="w-3 h-3 text-yellow-600 mr-1" />Book now</>;
      case "overdue":
        return <><AlertTriangle className="w-3 h-3 text-red-600 mr-1" />Overdue</>;
      case "due_this_week":
        return <><AlertTriangle className="w-3 h-3 text-yellow-600 mr-1" />Book now</>;
      case "quotes_ready":
        return <><MessageSquare className="w-3 h-3 text-blue-600 mr-1" />Quotes ready</>;
      case "completed":
        return <><CheckCircle className="w-3 h-3 text-gray-600 mr-1" />Completed</>;
      default:
        return <><AlertTriangle className="w-3 h-3 text-yellow-600 mr-1" />Book now</>;
    }
  };
  ...
  const propertyMaintenanceTemplates = [
    { id: 'boiler', icon: Flame, title: 'Boiler Service', type: 'maintenance', recurring: 'annual', cost: '£80-120' },
    { id: 'gutter', icon: Building, title: 'Gutter Clean', type: 'maintenance', recurring: 'biannual', cost: '£120-180' },
    { id: 'garden', icon: Home, title: 'Garden Work', type: 'maintenance', recurring: 'seasonal', cost: '£150-250' },
    { id: 'eicr', icon: Shield, title: 'EICR Check', type: 'safety', recurring: '5years', cost: '£200-350' },
    { id: 'custom', icon: Plus, title: 'Custom Trade Job', type: 'custom', recurring: 'never', cost: 'TBC' }
  ];

  const householdTemplates = [
    { id: 'bins', icon: Trash2, title: 'Bin Day', type: 'household', recurring: 'weekly', cost: 'Free' },
    { id: 'rent', icon: CreditCard, title: 'Pay Rent', type: 'financial', recurring: 'monthly', cost: 'TBC' },
    { id: 'meter', icon: Zap, title: 'Meter Reading', type: 'household', recurring: 'quarterly', cost: 'Free' },
    { id: 'council-tax', icon: Building, title: 'Council Tax', type: 'financial', recurring: 'monthly', cost: 'TBC' },
    { id: 'custom-task', icon: Plus, title: 'Custom Task', type: 'custom', recurring: 'never', cost: 'TBC' }
  ];

  // Additional household task categories
  const councilUtilitiesTemplates = [
    { id: 'bins-weekly', icon: Trash2, title: 'Bin Collection', type: 'household', recurring: 'weekly', cost: 'Free' },
    { id: 'council-tax', icon: Building, title: 'Council Tax', type: 'financial', recurring: 'monthly', cost: '£120/month' },
    { id: 'water-rates', icon: Zap, title: 'Water Rates', type: 'financial', recurring: 'monthly', cost: '£35/month' },
    { id: 'tv-licence', icon: Settings, title: 'TV Licence', type: 'financial', recurring: 'annual', cost: '£159/year' },
    { id: 'meter-reading', icon: Zap, title: 'Meter Readings', type: 'household', recurring: 'quarterly', cost: 'Free' }
  ];

  const propertyAdminTemplates = [
    { id: 'mortgage', icon: CreditCard, title: 'Mortgage Payment', type: 'financial', recurring: 'monthly', cost: 'TBC' },
    { id: 'ground-rent', icon: Building, title: 'Ground Rent', type: 'financial', recurring: 'annual', cost: 'TBC' },
    { id: 'service-charge', icon: Settings, title: 'Service Charge', type: 'financial', recurring: 'quarterly', cost: 'TBC' },
    { id: 'buildings-insurance', icon: Shield, title: 'Buildings Insurance', type: 'financial', recurring: 'annual', cost: '£300/year' },
    { id: 'contents-insurance', icon: Home, title: 'Contents Insurance', type: 'financial', recurring: 'annual', cost: '£150/year' }
  ];

  const seasonalTemplates = [
    { id: 'winter-heating', icon: Flame, title: 'Winter: Check Heating', type: 'maintenance', recurring: 'annual', cost: 'Free' },
    { id: 'spring-gutters', icon: Building, title: 'Spring: Clear Gutters', type: 'maintenance', recurring: 'annual', cost: '£80-120' },
    { id: 'summer-painting', icon: Home, title: 'Summer: Exterior Check', type: 'maintenance', recurring: 'annual', cost: '£200-500' },
    { id: 'autumn-chimney', icon: Flame, title: 'Autumn: Chimney Sweep', type: 'maintenance', recurring: 'annual', cost: '£60-100' }
  ];
                      ...
                      <Button
                        ...
                        className={`px-3 py-1 text-xs ${
                          filterType === filter.value 
                            ? filter.value === 'safety' ? 'bg-red-500 text-white' :
                              filter.value === 'maintenance' ? 'bg-orange-500 text-white' :
                              filter.value === 'financial' ? 'bg-blue-500 text-white' :
                              filter.value === 'household' ? 'bg-green-500 text-white' :
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="space-y-2">
                                {propertyMaintenanceTemplates.slice(0, 4).map(template => {
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {householdTemplates.slice(0, 4).map(template => {
                                  const IconComponent = template.icon;
                                  return (
                                    <Button
                                      key={template.id}
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {calendarDays.map((day, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className={`min-h-[100px] p-2 border border-gray-100 hover:bg-gray-50 cursor-pointer ${!isCurrentMonth ? 'text-gray-400 bg-gray-50' : ''} ${isToday ? 'bg-primary/10 border-primary' : ''}`}
                        >
                          ...
                          <div className="space-y-1">
                            {dayEvents.slice(0, 2).map((event) => (
                              <div
                                key={event.id}
                                className={`text-xs px-2 py-2 rounded border ${getStatusBorder(event.status)} cursor-pointer hover:shadow-sm transition-shadow`}
                                title={`${event.time} - ${event.title}\n${event.description}`}
                                onClick={() => toggleEventSelection(event.id)}
                              >
                                <div className="font-medium text-gray-900 truncate">{event.title}</div>
                                <div className="text-xs text-gray-600 flex items-center justify-between mt-1">
                                  <span>{event.time} • {formatCost(event.cost, event.status)}</span>
                                </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1">
                              ...
                              <span className="text-xs text-gray-600">{event.time} • {formatCost(event.cost, event.status)}</span>
                            </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        {overdueEvents.map((event) => (
                          <Card key={event.id} className={`${getBoardStatusBorder(event.status)} shadow-sm`}>
                            <CardContent className="p-3">
                              ...
                              <p className="text-xs text-gray-600 mb-2">{event.time} • {formatCost(event.cost, event.status)}</p>
                              <div className="text-xs text-red-600 mb-3 flex items-center">{getStatusIcon(event.status)}</div>
                              <Button size="sm" variant="secondary" className="w-full text-xs px-2 py-1.5 h-7 bg-gray-100 hover:bg-gray-200">
                                Get quotes
                              </Button>
                            </CardContent>
                          </Card>
                    ...
                    <Card className="bg-white border border-gray-200">
                      <CardHeader className="pb-3 border-b border-gray-100">
                        <CardTitle className="text-base font-semibold text-gray-900 flex items-center gap-2">
                          <CheckCircle className="w-5 h-5 text-green-600" /> Confirmed ({filteredEvents.filter(e => e.status === 'confirmed').length})
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-2">
                        {filteredEvents.filter(e => e.status === 'confirmed').map((event) => (
                          <Card key={event.id} className={`${getBoardStatusBorder(event.status)} shadow-sm`}>
                            <CardContent className="p-3">
                              <h4 className="text-sm font-medium mb-1">{event.title}</h4>
                              <p className="text-xs text-gray-600 mb-2">{event.time} • {event.cost}</p>
                              <div className="text-xs text-green-600 mb-3 flex items-center"><CheckCircle className="w-3 h-3 text-green-600 mr-1" />{event.contractor}</div>
                    ...
                    <Card className="bg-white border border-gray-200">
                      <CardHeader className="pb-3 border-b border-gray-100">
                        <CardTitle className="text-base font-semibold text-gray-900 flex items-center gap-2">
                          <CheckCircle className="w-5 h-5 text-gray-600" /> Completed ({filteredEvents.filter(e => e.status === 'completed').length})
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-2">
                        {filteredEvents.filter(e => e.status === 'completed').map((event) => (
                          <Card key={event.id} className={`${getBoardStatusBorder(event.status)} shadow-sm opacity-75`}>
                            <CardContent className="p-3">
                              ...
                              <p className="text-xs text-gray-500 mb-2">{event.time} • {event.cost}</p>
                              <div className="text-xs text-gray-500 flex items-center"><CheckCircle className="w-3 h-3 text-gray-600 mr-1" />Completed</div>
                            </CardContent>
                          </Card>
                        ))}
                      </CardContent>
                  ...
                  <div key={index} className="flex items-center justify-between hover:bg-gray-50 -mx-2 px-2 py-1.5 rounded">
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${
                        ...
                        item.status === 'expiring' ? 'bg-yellow-500' :
                        'bg-green-500'
                      }`}></div>
                    ...
                    <Badge 
                      ...
                      className={`text-xs px-2 py-1 ${
                        item.status === 'due' ? 'bg-red-100 text-red-800' :
