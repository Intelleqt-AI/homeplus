...
const Sidebar = () => {
    ...
    <>
      ...
      <div className="flex items-center gap-2 mb-8">
        <svg width="24" height="22" viewBox="0 0 28 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M3 10L14 2L25 10V22C25 22.5304 24.7893 23.0391 24.4142 23.4142C24.0391 23.7893 23.5304 24 23 24H5C4.46957 24 3.96086 23.7893 3.58579 23.4142C3.21071 23.0391 3 22.5304 3 22V10Z"
            stroke="#FBBF24"
            strokeWidth="2.5"
            strokeLinecap="round"
          ...
          {sidebarItems.map(item => {
            ...
            return (
              <li key={item.path}>
                ...
                >
                  <item.icon
                    className={`w-[18px] h-[18px] transition-colors duration-200 ${isActive ? 'text-white' : 'text-[#4A4A4A] group-hover:text-[#1A1A1A]'}`}
                    strokeWidth={1.5}
                  />
                  ...
                  {item.badge != null && item.badge > 0 && (
                    <span className={`min-w-[18px] h-[18px] text-[10px] font-bold rounded-full flex items-center justify-center px-1 leading-none ${
                      isActive ? 'bg-white text-[#1A1A1A]' : 'bg-[#FBBF24] text-white'
                    }`}>
      ...
      <div className="flex gap-3 items-center justify-between pt-4 border-t border-[#E8E8E3]">
        ...
        <LogOut
          ...
          className="w-5 h-5 cursor-pointer text-[#6B6B6B] hover:text-[#1A1A1A] transition-colors"
          size={10}
        />
      </div>
    </>
  ...
  return (
    <>
      ...
      <nav
        className={`lg:hidden fixed left-0 top-14 bottom-0 w-[280px] bg-[#F5F5F0] border-r border-[#E8E8E3] z-50 flex flex-col p-5 transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {navContent}
      </nav>
